module Api
    ...
    class ReservationsController < ApplicationController
        ...
        before_action :authenticate_user, only: [:get_all_my_reservations, :create]
        MAX_PAGINATION_LIMIT = 100
       
        def get_all_reservations
            reservations = Reservation.all.limit(limit).offset(params[:offset]).order(created_at: :asc)
            render json: ReservationsRepresenter.new(reservations).as_json 
        end

        def get_all_my_reservations
            reservations = Reservation.all.where("customer_id = #{helpers.current_user.id}")
            render json: ReservationsRepresenter.new(reservations).as_json 
        end

        def create
            hotel = Hotel.find_by(slug: params[:slug])
            reservation = Reservation.new(customer_id: helpers.current_user.id, hotel_id: hotel.id, date_from: params[:date_from] , number_of_days: params[:number_of_days])    
            if reservation.save
